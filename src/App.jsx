...
const callClaude = async (system, userMsg) => {
  ...
  try {
    console.log("📤 Sending to backend:", { 
      ...
      userMsgLength: userMsg?.length || 0,
      hasSystem: !!system,
      hasUserMsg: !!userMsg
    });
    const body = JSON.stringify({ system, userMsg });
    ...
    if (!r.ok) {
      ...
      throw new Error(d.error?.message || d.error || "Backend API request failed");
    }
    return d.result;
  } catch (backendError) {
    // Fallback to direct API call if API key is provided
    if (!apiKey) {
      throw new Error("Backend server not running and no API key configured. Start the backend with: npm run dev:backend");
    }
    
    console.warn("Backend unavailable, using direct Groq API call");
    const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "llama-3.3-70b-versatile", max_tokens: 1000, messages: [{ role: "system", content: system }, { role: "user", content: userMsg }] })
    });
    const d = await r.json();
    if (!r.ok) {
      throw new Error(d.error?.message || "Groq API request failed");
    }
...
const parseDOCX = async (file) => {
  try {
    ...
    // Use JSZip to read DOCX (which is a ZIP file)
...
const parseDoc = async (file) => {
  // Legacy .doc files - attempt to extract text
  const text = await file.text();
...
function UploadZone({ onText }) {
  ...
  if (paste) return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {error && <div style={{ padding: "12px 16px", background: "rgba(251,113,133,0.1)", border: "1px solid rgba(251,113,133,0.3)", borderRadius: 10, fontSize: 13, color: "rgba(251,113,133,1)", fontFamily: "'Outfit',sans-serif" }}>{error}</div>}
      <textarea value={txt} onChange={e => { setTxt(e.target.value); setError(""); }} placeholder="Paste your full resume text here…"
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <Btn onClick={() => { if (!txt.trim()) { setError("Please paste your resume content first."); return; } onText(txt); }} full>Analyze Resume →</Btn>
        <Btn variant="ghost" onClick={() => { setPaste(false); setError(""); }}>Back</Btn>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const doAnalyze = async (text, r, l) => {
    ...
    catch (e) { setErr(e.message || "Analysis failed — please try again."); setStage("upload"); }
  };
    ...
    <div style={{ minHeight: "100vh", color: C.text, position: "relative" }}>
      ...
      <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => setShowSettings(!showSettings)} style={{ padding: "6px 12px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: apiKey ? C.teal : C.rose, fontSize: 12, cursor: "pointer", fontFamily: "'Outfit',sans-serif", fontWeight: 600, transition: "all 0.2s" }}>⚙️ {apiKey ? "Settings" : "API Key"}</button>
      ...
      {showSettings && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" }}>
          <div className="glass-card" style={{ maxWidth: 400, width: "90%", padding: "28px 32px", background: "rgba(5,8,16,0.95)", border: "1px solid rgba(78,236,200,0.3)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 24, color: C.text, margin: 0 }}>API Configuration</h2>
              <button onClick={() => setShowSettings(false)} style={{ background: "none", border: "none", color: C.textSec, fontSize: 24, cursor: "pointer" }}>✕</button>
            </div>
            
            {apiKey && (
              <div style={{ background: "rgba(78,236,200,0.1)", border: "1px solid rgba(78,236,200,0.3)", borderRadius: 12, padding: 12, marginBottom: 16 }}>
                <div style={{ fontSize: 11, color: C.teal, fontFamily: "'Outfit',sans-serif", fontWeight: 600, marginBottom: 4 }}>✓ API KEY CONFIGURED</div>
                <div style={{ fontSize: 12, color: C.textSec, fontFamily: "'Outfit',sans-serif" }}>Key: {apiKey.slice(0, 10)}...{apiKey.slice(-4)}</div>
              </div>
            )}

            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 12, color: C.textSec, fontFamily: "'Outfit',sans-serif", fontWeight: 600, marginBottom: 8 }}>Anthropic API Key</label>
              <input 
                type="password"
                value={apiKeyInput}
                onChange={e => setApiKeyInput(e.target.value)}
                placeholder={apiKey ? "Enter new key to update..." : "sk-ant-xxxxxxxxxxxxx"}
                style={{ 
                  width: "100%", 
                  background: "rgba(255,255,255,0.04)", 
              ...
              />
              <div style={{ fontSize: 11, color: C.textMut, fontFamily: "'Outfit',sans-serif", lineHeight: 1.6, marginBottom: 16 }}>
                Get your free API key from <span style={{ color: C.blue }}>console.anthropic.com/keys</span>. Never share your key publicly.
              </div>
            </div>

            <div style={{ display: "flex", gap: 10 }}>
              <Btn onClick={saveApiKey} full>Save API Key</Btn>
              <Btn variant="ghost" onClick={() => { setShowSettings(false); setApiKeyInput(""); }}>Cancel</Btn>
            </div>
